...
- **L2 索引周期审计（运行期自愈）**：新增隐藏配置 `l2_index_audit_interval_sec`（默认 600 秒，0 禁用）。适配器初始化成功后启动周期审计任务，每轮对 SQLite 与 FAISS 索引做一次廉价的 ID 集比较，发现漂移自动增量修复并原子落盘；shutdown 时干净取消。启动对账失败的索引不再等待下次重启，而是在 Provider 恢复后的下一轮审计中自动补全。

### Fixed

- **仓库既有缺陷检查**：均衡抽样在候选数不超过上限时仍执行群/用户配额；遗忘评分与 L2/L3 保留期判断支持带时区的时间戳，并防止远期时间导致指数溢出。
- **配置恢复与工作队列**：删除隐藏配置覆盖值时通知监听器实际默认值；运行中任务的后续请求合并保留最高优先级、最早等待时间与顺序，避免低优先级更新延误已排队的紧急工作。
- **画像与隐藏配置写接口**：非法 JSON/非对象请求返回 400；群画像删除正确读取请求体中的人格，并保留查询参数优先级；浮点配置拒绝布尔值、NaN 和无穷值。删除接口不再因 URL 已提供 ID 而忽略损坏的请求体。
- **基础静态检查**：清理 7 项既有的未使用导入、未使用赋值和歧义变量命名问题。
- **同步 Iris Chat Memory 0.3.0 的适用修复**（[核对记录](MEMORY_SYNC_2026-09-06.md)）：保留本插件的混合检索、归档、人格隔离、梦境游标与相对导入结构。
- **Embedding 实际维度校验**：初始化通过真实向量探测维度，声明维度不准确或历史元数据缺失时按实际输出迁移；探测失败不再回退为 384 维，允许后续自动恢复重试。写入、批量写入、检索、更新与归档恢复在进入 FAISS 前校验维度。
- **L2 索引恢复并发边界**：补回向量前复查记录是否已删除或内容已编辑；摘除脏向量前复查槽位是否已被重新占用；即使索引 ID 集一致也清理过期 free-list，并按最新 SQLite 状态去重保存。每批恢复立即落盘，后续批次取消时保留已完成进度。
- **模型迁移完整性与备份保留**：导出不完整时保留旧库，导入不完整或最终索引/元数据落盘失败时保留正式记忆 JSON 与归档 SQLite 快照；仅完整导入、恢复归档并成功落盘后删除备份。
- **L2 关闭与数据库访问**：关闭连接前等待 checkpoint 和进行中的索引对账；数据库辅助方法在取得锁后复查连接状态。
- **LLM 与图谱工具兼容**：框架 `llm_generate` 的字典历史消息转换为标准 `Message`，不修改调用方输入；Provider 获取拒绝非聊天类型；`search_memory(with_graph_context=True)` 只拼接图谱格式化结果中的文本，避免返回元组导致整个检索失败。
- **Web 参数校验**：L1 查询缺少 `group_id` 时返回 400，保留显式空会话键兼容性；群画像更新收到 JSON `null` 时返回 400，避免空值访问导致 500。
- **L2 重启/热重载后向量检索失效（需手动「编辑保存」逐条恢复）**：SQLite 为 WAL 即时持久，FAISS 索引依赖懒落盘（每 50 次写入 checkpoint、关闭时保存）。进程被直接结束（Windows 桌面端关闭窗口、崩溃、插件热重载取消后台初始化）后索引落后于 DB；而原启动对账是一次性的，任何一批重嵌入失败（启动期 Embedding Provider 未预热/网络未就绪/限流）即整体放弃且运行期不再重试——适配器仍报「初始化成功」，缺失向量的记忆从此检索不到，只能靠 Web 面板手动编辑保存触发单条重嵌入恢复。现对账改为增量修复：脏向量（索引有、DB 无）直接 `remove_ids` 摘除；缺失向量（DB 有、索引无）仅重嵌入缺失行，嵌入调用带 3 次指数退避重试，批间进度持久化、可在下轮续跑；与并发写入路径竞争时复查索引 ID 避免同槽位重复写入。附带修复：对账落盘时过滤 free-list 中已占用的槽位，避免崩溃恢复后新写入复用活动槽位覆盖既有记忆。

### Tests

- 仓库既有缺陷检查新增 47 项回归及兼容性用例；最终全量 1903 passed（19 条 AstrBot 依赖弃用警告）。全仓库 Ruff 基础规则 `E4,E7,E9,F` 和 `git diff --check` 通过；前端 12 项测试、TypeScript 检查及生产构建通过（图谱依赖分块仍有体积提示）。
- 此前同步新增 31 项回归覆盖，复现并修复移植测试中的 17 个失败用例；同步完成时全量 1856 passed。该批修改的 Python 文件均通过 Ruff，`git diff --check` 通过。
...
- 本次更新涉及 LLM 工具注册方式重构、qq_official 平台适配器接入与 L2 关键词索引启动全量重建，**需要完全重启 AstrBot 才能生效**，不建议仅热重载本插件。
...
- 平台注册表对齐 AstrBot 4.x 实际协议名：`qqofficial` 更名 `qq_official`（由本次新增的 `QQOfficialAdapter` 接管，不再降级 GenericAdapter），删除 `qq`/`gewechat` 死条目；telegram、webchat、wecom 等其余未注册平台行为不变，统一经 GenericAdapter 降级。

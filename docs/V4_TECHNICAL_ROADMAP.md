...
> 状态：目标设计与最终验收标准；当前开发候选的覆盖范围见实施报告。
>
...
2026-09-08 开发快照中的 SDK 已使用 httpx 原生异步连接池，并提供整次调用 deadline、取消与 `aclose()`；插件已消费这些公共接口。旧 urllib 线程限制不再适用于摘要对应的安装物。SSE 仍未新增，页面使用按需查询。包版本号未变，不能只凭 `0.11.1` 推断具备新接口。
...
## 19. 本轮落地约定（2026-09-08）

- 本地公共入口为 `iris_memory_core.embedded` 与 `embedded_providers`，远程入口为独立 `iris_memory_sdk`，均从安装 wheel 消费。插件不分发 Python 包。
- 当前 Core 缺少独立 Space provisioning：暂按会话/人格分配 Agent/Space，共用一个 Runtime，公开 128 组合上限；远程配置禁止跨组合共享 Agent。人格修订保持注册键稳定。
- 完整人格通过 AstrBot 公共 PersonaManager 与异步会话配置选择执行镜像，保留原工具/技能限制；停用恢复原选择，不通过字符串删除宿主规则。
- 当前代码一律跳过 Observation，仅采用事实、任务、关注点等候选。“原始观察都已在宿主历史中出现”的假设对群聊旁观消息不成立，属于需要按第 20 节修正的缺项，不能作为长期设计原则。
- 本地自动认知暂被新发现的 Scope 接缝问题阻塞；显式记忆可用，不采用模型生成授权字段或私有数据库读写补洞。
- Pages、临时测试 harness、核心自动化测试均已落地。正式宿主、真实模型、检索质量对照与 24 小时门禁保持待验证。

## 20. Observation 与多人场景修订（待实施）

以下修订作为下一轮实施约定，详细字段、工作包和验收分别见 [插件方案](V4_OBSERVATION_REVISION_PLAN.md) 与 [Core 方案](CORE_MULTIPARTY_REVISION_PLAN.md)。本次只更新方案文档，不修改业务代码或 Core。

1. 环境消息使用已有 Observation；不新增 EnvironmentMemory 对象、角色或独立事实库。RecentContext 恢复近期环境，Core 后台将有价值内容加工为 Episode/Claim/Relation 等。
2. 采集不依赖消息是否触发机器人回复。是否收集、是否用于本轮上下文、是否提炼长期记忆分别控制；所有开关仍在 Pages。
3. 稳定主体对应 Agent，来源环境对应 Space，会话/场次对应可选 Session。当前每“会话/人格”一个 Agent 的方式仅为开发候选受限实现；不能自动合并旧历史来切换模型。
4. 采集传递稳定发言者、来源事件和时间；回复对象、@ 与 Agent 信号通过 Core 的版本化结构约定传递。昵称不决定身份，旁观不等于参与，收到消息不等于认同其内容。
5. ContextBuilder 使用 Core 公开 Recent/Recall 和来源信息补足宿主缺失背景，按来源与摘要覆盖去重；保留当前有效事实和历史发言的区别，取消 Observation 全量排除。
6. Core 对多人认知输入提供有界参与者映射，服务端绑定固定 Scope、身份及证据版本；插件不代填授权字段。身份快照、候选主体、异步 Provider 和远程身份注册是独立验收点。
7. 群聊、直播间等多个来源并行观察时，Core 要明确入站来源授权与 Active Surface 发言权的关系；插件不得通过抢租约、改角色或绕过 SDK 实现接收。
8. 继续采用 SQLite、有界队列/窗口/缓存、批处理与单 Runtime。新接缝以实际 Core/SDK 安装产物和双入口多人测试为验收依据，不能将契约草案写成已可调用。

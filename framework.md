...
## 九、记忆可见性机制（Memory Scope）

### 9.1 四层记忆可见性模型

为了支持更灵活的记忆共享机制，系统引入 **记忆可见性范围（Memory Scope）**，实现从个人到全局的多层级可见性控制：

```
┌─────────────────────────────────────────────────────────────┐
│  层级1: USER_PRIVATE（用户私有记忆）                        │
│  • 仅创建者可见                                             │
│  • 适用：个人隐私信息、私聊中的个人记忆                     │
│  • 查询：任何场景只返回给创建者                             │
├─────────────────────────────────────────────────────────────┤
│  层级2: GROUP_SHARED（群组共享记忆）                        │
│  • 群组内所有成员可见                                       │
│  • 适用：群组规则、公共知识、项目协作信息                   │
│  • 查询：任何人在该群组都能看到                             │
├─────────────────────────────────────────────────────────────┤
│  层级3: GROUP_PRIVATE（群组个人记忆）[默认]                 │
│  • 仅创建者在该群组可见                                     │
│  • 适用：用户在群组中的个人偏好、记录                       │
│  • 查询：仅创建者在该群组可见，跨群组隔离                   │
├─────────────────────────────────────────────────────────────┤
...
**群聊场景查询顺序：**
```
1. GROUP_SHARED（群组共享记忆）
   ↓
2. GROUP_PRIVATE（用户在该群组的个人记忆）
   ↓
3. GLOBAL（全局记忆）
```
...
{
  "user_id": "用户A",
  "group_id": "群组123",  // USER_PRIVATE/GLOBAL 时为 null
  "scope": "group_private",  // 四种类型之一
  "type": "fact",
  "content": "记忆内容"
}
```
...
**场景1：群管理员设置群规则（GROUP_SHARED）**
```python
memory = Memory(
    user_id="管理员",
    group_id="群组123",
    scope=MemoryScope.GROUP_SHARED,
    content="群规：禁止广告、尊重他人",
    type=MemoryType.RULE
)
# 结果：所有群成员都能看到这条规则
```

**场景2：用户记录个人偏好（GROUP_PRIVATE）**
```python
memory = Memory(
    user_id="用户A",
    group_id="群组123",
    scope=MemoryScope.GROUP_PRIVATE,  # 默认
    content="我喜欢科幻电影",
    type=MemoryType.PREFERENCE
)
# 结果：仅用户A在群组123中能看到，其他成员看不到
```
...
memory = Memory(
    ...
    content="系统维护通知：今晚10点升级",
    type=MemoryType.ANNOUNCEMENT
)
# 结果：所有用户在任何场景都能看到
```
...
- **默认行为**：`GROUP_PRIVATE`，与之前逻辑一致
- **数据迁移**：现有数据自动继承 `GROUP_PRIVATE` scope
- **配置选项**：支持通过 `_conf_schema.json` 设置默认 scope
...
**文档版本**：v1.1  
**最后更新**：2026-02-02  
**兼容版本**：AstrBot Plugin Iris Memory v0.1.0+

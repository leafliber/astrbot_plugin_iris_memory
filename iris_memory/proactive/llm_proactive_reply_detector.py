...
class LLMProactiveReplyDetector(LLMEnhancedBase):
    """LLM增强主动回复检测器
    ...
    async def detect(
        ...
        if self._mode == DetectionMode.RULE:
            return self._rule_detect(messages, user_id, group_id, context)
        elif self._mode == DetectionMode.LLM:
            return await self._llm_detect(messages, user_id, group_id, context)
        else:
            return await self._hybrid_detect(messages, user_id, group_id, context)
        ...
        try:
            ...
            if loop.is_running():
                ...
                decision = loop.run_until_complete(
                    self._rule_detector.analyze(messages, user_id, group_id, context)
                )
        except Exception:
            decision = self._sync_rule_detect(messages, user_id, group_id, context)
    ...
    def _sync_rule_detect(
        ...
        signals = self._rule_detector._detect_reply_signals(last_message, "\n".join(messages))
        ...
        if reply_score >= 0.5:
            return ProactiveReplyDecision(
                should_reply=True,
                urgency=ReplyUrgency.HIGH if reply_score >= 0.7 else ReplyUrgency.MEDIUM,
                reason=" + ".join(reasons),
    ...
    async def _rule_detect_async(
        ...
        """规则检测（异步版本）"""
        decision = await self._rule_detector.analyze(messages, user_id, group_id, context)
    ...
    async def _llm_detect(
        ...
        """LLM检测"""
        messages_text = "\n".join([f"用户: {m}" for m in messages[-5:]])
        prompt = PROACTIVE_REPLY_PROMPT.format(messages=messages_text)
        result = await self._call_llm(prompt)
        
        if not result.success or not result.parsed_json:
            logger.debug(f"LLM proactive reply detection failed, falling back to rule")
            return await self._rule_detect_async(messages, user_id, group_id, context)
    ...
    async def _hybrid_detect(
        ...
        """混合检测：规则预筛 + LLM确认"""
        rule_result = await self._rule_detect_async(messages, user_id, group_id, context)
        
        if rule_result.urgency == ReplyUrgency.CRITICAL:
            return rule_result
        
        if rule_result.urgency == ReplyUrgency.IGNORE and rule_result.confidence >= 0.8:
            last_message = messages[-1] if messages else ""
            if self._might_need_reply_anyway(last_message):
                llm_result = await self._llm_detect(messages, user_id, group_id, context)
    ...
    def _parse_llm_result(self, data: Dict[str, Any]) -> LLMReplyDecision:
        ...
        urgency = urgency_map.get(urgency_str, ReplyUrgency.IGNORE)
        ...
        if isinstance(suggested_delay, (int, float)):
            suggested_delay = max(0, min(120, int(suggested_delay)))
        else:
            suggested_delay = 30
